...
int main() {
    ...
    for (int i = 1; i < argc; i++) {
        ...
        else if (strcmp(argv[i], CMD_NAME_SHORT) == 0 || strcmp(argv[i], CMD_HELP_LONG) == 0) {
            name_found = fread("sys/name.txt", buffer_name, sizeof(buffer_name));
            if (name_found == -1) {
        ...
        else if (strcmp(argv[i], CMD_VERSION_SHORT) == 0 || strcmp(argv[i], CMD_VERSION_LONG) == 0) {
            version_found = fread("sys/version.txt", buffer_version, sizeof(buffer_version));
            if (version_found == -1) {
        ...
        else if (strcmp(argv[i], CMD_ARCH_SHORT) == 0 || strcmp(argv[i], CMD_ARCH_LONG) == 0) {
            arch_found = fread("sys/arch.txt", buffer_arch, sizeof(buffer_arch));
            if (arch_found == -1) {
